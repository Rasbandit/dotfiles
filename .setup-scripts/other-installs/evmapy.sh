...
echo 'ACTION=="add", KERNEL=="event[0-9]*", RUN+="/usr/bin/pkill -HUP -f evmapy"' | sudo tee /etc/udev/rules.d/99-evmapy-reload.rules >/dev/null && sudo udevadm control --reload-rules && sudo udevadm trigger --subsystem-match=input
